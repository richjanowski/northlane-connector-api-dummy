...
Northlane's dashboard defines five built-in roles. These are relevant to integrators because webhook payloads and audit log entries include the `actor_role` of whoever triggered a change.
...
| Role | Default scopes |
|---|---|
| `owner` | `reservations:read`, `reservations:write`, `resources:read`, `resources:write`, `webhooks:manage`, `reports:read` |
